...
function isDate2Greater(date1, date2){
	...
	// Function to chech the data fetched is not more than 7 days
	function isDateDiffValid(date1, date2, maxDaysAllowed){
		var start = new Date(date1);
		var end = new Date(date2);
		var ONE_DAY = 1000 * 60 * 60 * 24
		var diff = Math.abs(end - start)
	    var final_diff=diff/ONE_DAY;
		if (final_diff > (maxDaysAllowed-1)){
			alert("You have selected date range beyond the configured duration of "+maxDaysAllowed+" days");
			return false;
		}
...
//added by Ganesh Nyati for PricePoint changes on 3rd sept 2012  	
//allowed only alphanumeric , # and . value	
	function isValidMsg(){
		var c= event.keyCode;
		event.keyCode=(!((c>=65 && c<=90) ||(c>=97 && c<=122)|| (c>=48 && c<=57) || (c==32)||(c==44)||(c==188)||(c==58) ||(c==46)|| (c==35) ))?0:event.keyCode;
	}
	
	function isValidMsgFormat(message){
		var strValidChars = "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789.#,:  ";
		var blnResult = true;
  		//  test message consists of valid characters listed above
   		for (i = 0; i < message.length && blnResult == true; i++){
      		strChar = message.charAt(i);
      		if (strValidChars.indexOf(strChar) == -1){
    		     blnResult = false;
         	}
      	}
	...
	function maxLengthValidity(message,maxLength){
		...
		/* Function to chech request date selected by user on GUI is not less than (current date - maxDaysAllowed) days
		Added by Ganesh on 03-09-2013 for CustomerTransactionRpt Changes for Bharti Store*/
	function isDateRangeValid(date1,maxDaysAllowed){
		var start = new Date(date1);
		var sysDate = new Date();
		if(start > sysDate){
			alert("Date cannot be more than today");
			return false;
		}
		var ONE_DAY = 1000 * 60 * 60 * 24
		var diff = Math.abs(sysDate - start)
	    var final_diff=diff/ONE_DAY;
		if (final_diff > maxDaysAllowed){
			alert("You have selected date beyond the configured duration of "+maxDaysAllowed+" days");
			return false;
		}
		return true;
	}

	/* Function to validate Customer Account Number.Number should contain alphanumeric,Hypen(-) and UnderScore(_) values only.	*/
	...
	function isValidAccountNumber(){
		...
		event.keyCode=(!((c>=65 && c<=90)||(c>=97 && c<=122)||(c>=48 && c<=57)||c==45||c==95))?0:event.keyCode;
	}
	
	/*Function to validate  */
	function checkAccountNumber(strString) {
