<%@ taglib uri="/WEB-INF/struts-html.tld" prefix="html"%>
<%@ taglib uri="/WEB-INF/struts-bean.tld" prefix="bean"%>
<%@ taglib uri="/WEB-INF/struts-logic.tld" prefix="logic"%>
<HTML>
<BODY background="<%=request.getContextPath()%>/images/bg_main.gif"  bgcolor="#FFFFFF" leftmargin="0" topmargin="0" marginwidth="0"
	marginheight="0">

<table width="167" border="0" cellspacing="0" cellpadding="0" height="100%" valign="top">
 <tr bgcolor="#FFFFFF"> 
    <td valign="top" bgcolor="#B00000" style="background-image:url(<%=request.getContextPath()%>/images/left_bg.gif); background-repeat: no-repeat" width="167" align="left"> 
      <table width="167" border="0" cellspacing="0" cellpadding="0">
		<tr> 
          <td valign="top"><html:link
			action="ChangePasswordAction.do?methodName=initNonContext"
			styleClass="menu"><img src="<%=request.getContextPath()%>/images/but03.gif" width="168" height="37" border="0"></html:link></td>
        </tr>
        
		<tr> 
          <td valign="top"><a href="#"><img src="<%=request.getContextPath()%>/images/but05.gif" width="168" height="37" border="0"></a></td>
        </tr>
	
        <tr> 
          <td><table width="162" height="168" border="0" cellpadding="3" cellspacing="0" >
                <tr>
                  <td align="center"><img src="<%=request.getContextPath()%>/images/spacer.gif" height="10"></td>
                </tr>
                <tr>
                  <td align="center"><br>
                      </td>
                </tr>
              </table></td>
        </tr>
      </table>
    </td>
  </tr>
</TABLE>
</BODY>
</HTML>
